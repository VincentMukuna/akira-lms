...
export function FillInBlankQuestionEditor({
  ...
  const getBlanksError = (blankIndex: number) =>
    errors[`data.questions.${index}.blanks.${blankIndex}.answer`];
    ...
    <div className="space-y-4">
          ...
          <Button
            ...
            onClick={() => {
              ...
              const cursorPosition =
                (document.activeElement as HTMLTextAreaElement)?.selectionStart || text.length;
              const newText =
                text.slice(0, cursorPosition) + '[blank]' + text.slice(cursorPosition);
      ...
      <div className="space-y-4">
        ...
        {(question.blanks || []).map((blank, blankIndex) => {
          ...
          const context =
            text.slice(start, blank.position?.start || 0) +
            '___________' +
            text.slice(blank.position?.end || 0, end);

          return (
            <div key={blank.id} className="rounded-md border p-4">
              <div className="mb-2 text-sm text-muted-foreground">...{context}...</div>
