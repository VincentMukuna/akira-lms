import {
  ...
  ModuleType,
  Section,
} from '@/components/CourseBuilder/types/course';
import { updateSectionOrder as updateSectionOrderApi } from '@/lib/api/course-builder';
...
export function useAddSection() {
  ...
  const mutation = useMutation({
    ...
    onMutate: async ({ course_id, title, order }) => {
      ...
      const previousContent = queryClient.getQueryData<CourseContent>([
        'course-content',
        course_id,
      ]);
...
export function useUpdateModule() {
  ...
  return useMutation({
    mutationFn: async (updateModuleData: UpdateModuleData) => {
      ...
      const courseContent = queryClient.getQueryData<CourseContent>([
        'course-content',
        updateModuleData.course_id,
      ]);
      if (!courseContent) {
      ...
      return courseBuilderApi.updateModule({
        id: updateModuleData.id,
        title: updateModuleData.title,
        data: updateModuleData.data,
      });
    },
    onMutate: async (updateModuleData) => {
      ...
      queryClient.setQueryData(['course-content', updateModuleData.course_id], (old: any) => ({
        ...old,
        modules: old.modules.map((m: BaseModule) =>
          m.id === updateModuleData.id ? updateModuleData : m,
        ),
      }));
      return { previousContent };
    },
    ...
    onSuccess: (updatedModule, updateModuleData) => {
      queryClient.setQueriesData(
        { queryKey: ['course-content', updateModuleData.course_id] },
        (old: any) => {
          if (!old) return old;
          return {
            ...old,
...
export function useUpdateModuleOrder() {
  ...
  return useMutation({
    ...
    onMutate: async (updateModuleOrderData) => {
      const previousContent = queryClient.getQueryData<CourseContent>([
        'course-content',
        updateModuleOrderData.course_id,
      ]);
      console.log('PREVIOUS CONTENT', previousContent);
      if (!previousContent) {
        throw new Error('Previous content not found');
      ...
      queryClient.setQueryData(['course-content', updateModuleOrderData.course_id], (old: any) => ({
        ...old,
        modules: old.modules.map((m: BaseModule) => {
          const updatedOrder = updateModuleOrderData.module_orders.find(
            (mo: ModuleOrder) => mo.id === m.id,
          );
    ...
    onSuccess: (updatedModules, updateModuleOrderData) => {
      queryClient.setQueriesData(
        { queryKey: ['course-content', updateModuleOrderData.course_id] },
        (old: any) => {
          if (!old) return old;
          return {
            ...old,
