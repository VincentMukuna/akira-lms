...
export default function CourseBuilder({ course_id, defaultCourseContent }: Props) {
  ...
  const handleAddModule = (section_id: string, type: ModuleType) => {
    ...
    const moduleCount =
      courseContent?.modules.filter((m) => m.section_id === section_id).length || 0;
    addModule({
      section_id,
      course_id,
      type,
      title: `Module ${moduleCount + 1}`,
      order: moduleCount + 1,
      data: null,
    });
  };
  ...
  return (
    <div className="flex h-[calc(100vh-4rem)]">
      ...
      <div className="w-80 border-r">
        <ScrollArea className="-ps-4 h-full pe-4">
          <SectionList
      ...
      <div className="flex-1">
        <ScrollArea className="h-full">
          {selectedModule ? (
            <div className="p-6">
              <ModuleFactory module={selectedModule} onChange={handleModuleUpdate} />
            </div>
          ) : (
            <div className="flex h-full items-center justify-center">
