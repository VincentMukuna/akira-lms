...
export const EditorProvider = ({
  ...
  return (
    <TooltipProvider>
      <div
        className={cn(className, 'prose dark:prose-invert [&_.ProseMirror-focused]:outline-none')}
      >
...
const BubbleMenuButton = ({ name, isActive, command, icon: Icon, hideName }: EditorButtonProps) => (
  <Button onClick={() => command()} variant="ghost" className="flex w-full gap-4" size="sm">
    <Icon size={12} className="shrink-0 text-muted-foreground" />
    {!hideName && <span className="flex-1 text-left">{name}</span>}
    {isActive() ? <CheckIcon size={12} className="shrink-0 text-muted-foreground" /> : null}
  </Button>
);
...
export const EditorSelector = ({
  ...
  return (
    <Popover modal open={open} onOpenChange={onOpenChange}>
      ...
      <PopoverContent sideOffset={5} align="start" className={cn('w-48 p-1', className)} {...props}>
        {children}
      </PopoverContent>
    </Popover>
  );
...
export const EditorTextColor = ({ color, name, hideName = false }: EditorTextColorProps) => {
  const { editor } = useCurrentEditor();
  ...
  return (
    <BubbleMenuButton
      ...
      icon={() => <div className="size-4 rounded-sm border" style={{ backgroundColor: color }} />}
...
export const EditorCharacterCount = {
  Characters({ children, className }: EditorCharacterCountProps) {
    ...
    return (
      <div
        className={cn(
          'absolute bottom-4 right-4 rounded-md border bg-background p-2 text-sm text-muted-foreground shadow',
          className,
        )}
      >
        {children}
